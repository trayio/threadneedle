...
function validateAndGetType ([ methodName, methodConfig ]) {
	const { type } = ( _.isFunction(methodConfig) ? { type: 'function' } : methodConfig );
...
module.exports = function (...addMethodArgs) {
	...
	switch (targetType) {
		...
		// case 'function': return addMethodSOAP.call(this, ...addMethodArgs);
		default:
