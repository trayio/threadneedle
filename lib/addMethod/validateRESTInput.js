...
module.exports = function (methodName, config) {
	...
	if (!methodName || !_.isString(methodName)) {
		throw new Error(`The first parameter passed to 'addMethod' should be a string. Method: ${ _.snakeCase(methodName)}`);
	}
	...
	if (!config || !_.isObject(config)) {
		throw new Error(`The 'config' object should be an object. Method: ${ _.snakeCase(methodName)}`);
	}
	...
	if (!config.url && config.url !== '') {
		throw new Error(`The 'url' config parameter should be declared. Method: ${ _.snakeCase(methodName)}`);
	}
	...
	if (_.isString(method)) {
		...
		throw new Error(`The 'method' parameter needs to be provided in the method configuration. Method: ${ _.snakeCase(methodName)}`);
	}
